...
class Ass_turma_prof(object):
	...
	def pegarAss_turma_profs(self, condicao, valores, inicio=0, quantidade=0):
		associacoes = []
		for associacao in BancoDeDados().consultarMultiplos("SELECT * FROM ass_turma_prof WHERE %s" % (condicao), valores):
			associacoes.append(ModelAss_turma_prof(associacao))
		return associacoes
	
	def pegarAss_turma_prof(self, condicao, valores):
		return ModelAss_turma_prof(BancoDeDados().consultarUnico("SELECT * FROM ass_turma_prof WHERE %s" % (condicao), valores))
