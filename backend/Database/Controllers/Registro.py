...
class Registro(object):
	...
	def pegarRegistro(self, condicao, valores, inicio=0, quantidade=0):
		registro = []
		for registro in BancoDeDados().consultarMultiplos("SELECT * FROM registro WHERE %s" % (condicao), valores):
			registro.append(ModelRegistro(registro))
		return registro
	
	def pegarRegistro(self, condicao, valores):
		return ModelRegistro(BancoDeDados().consultarUnico("SELECT * FROM registro WHERE %s" % (condicao), valores))
