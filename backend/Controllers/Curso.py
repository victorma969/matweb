...
class Curso(Controller):
	...
	def Listar(self,pedido_listar):
		return RespostaListar(BDCurso().pegarCursos("WHERE id_campus = %s AND id_grau = %s AND nome LIKE %s LIMIT %s OFFSET %s",(str(pedido_listar.getId_campus()),"%"+str(pedido_listar.getId_grau()),"%"+pedido_listar.getNome().replace(' ','%')+"%",str(pedido_listar.getQuantidade()),(str(pedido_listar.getQuantidade()*pedido_listar.getPagina())))))
