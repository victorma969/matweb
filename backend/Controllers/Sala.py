...
class Sala(Controller):
	...
	def Listar(self,pedido_listar):
		return RespostaListar(BDSala().pegarSala("WHERE id_predio = %s AND codigo LIKE %s LIMIT %s OFFSET %s",(str(pedido_listar.getId_predio()),"%"+pedido_listar.getCodigo().replace(' ','%')+"%",str(pedido_listar.getQuantidade()),(str(pedido_listar.getQuantidade()*pedido_listar.getPagina())))))
